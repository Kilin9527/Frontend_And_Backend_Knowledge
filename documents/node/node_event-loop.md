...
Node主要分为四大部分，Node Standard Library，Node Bindings，V8 引擎，Libuv(EventLoop)，架构图如下:
...
- **Node Bindings**: 负责JavaScript与C++代码的沟通，封装V8引擎和Libuv库，给Node Standard Library提供API支持。
...
### 2.2 什么是Event Loop
Node是基于JavaScript实现的，但是JavaScript本身是单线程，并不支持多线程，一旦遇到大量耗时任务则会阻塞JavaScript线程。为了解决这个问题，Node使用了一种叫做event loop的技术将这些耗时的，会阻塞JavaScript线程的任务交由event loop来处理，当event loop处理完成之后，通知JavaScript处理该事件对应的回调。这样，Node就拥有了多线程处理事件的能力。
...
当一个Node应用启动之后，它会初始化event loop，V8引擎开始解析并执行JavaScript代码，遇到异步任务，将其放入到event loop执行，event loop会根据异步任务的类型，分发到不同的Libuv线程中执行。
...
每个阶段都有一个执行回调的FIFO队列，直到队列中的回调被清空或达到最多执行次数限制之后，才会进入下一个阶段。
...
4. **poll**：
    4.1 检查新的I/O事件。
    4.2 执行除了close 回调s，定时器回调和setImmediate之外的所有回调。
    4.3 Node会在适当的时候阻塞。
...
### 2.3 阶段细节
#### Timers阶段 
设置定时器的等待时间和回调函数，一旦到期，timers会尽可能早一点执行定时器的回调函数，这个“早”是相对的，取决于操作系统当前执行的任务，分两种情况：

假设定时器设置的等待时间为x。
* case1: 如果到达定时器的等待时间，操作系统中没有其他任务在执行，可以立即执行定时器指定的回调。**实际等待时间为x**。
* case2: 如果到达定时器的等待时间，操作系统有其他任务正在执行，那么timers队列需要等待操作系统的其他任务执行完成之后，才可以开始执行timers中的回调，这时候实际等待的时间就会比预设的时间长。**实际等待时间为x+n**。
...
#### poll阶段
当event loop进入到poll阶段执行情况如下: 

1. 如果poll队列不为空，event loop会同步的迭代执行poll队列中的回调，直到poll队列为空或者达到执行次数上限为止。
2. 如果poll队列为空，且timers队列中的定时器到期，那么event loop会回到timers阶段去执行到期的回调。
3. 如果poll队列为空，且timers队列中没有到期的定时器，且设置了setImmediate()，event loop会结束poll阶段，进入check阶段，执行相应的回调。
4. 如果poll队列为空，且timers队列中没有到期的定时器，且没设置setImmediate()，event loop会等待其他的回调被添加到poll队列，新添加的回调会立即执行。

注：上文中的[2]与[3, 4]的顺序是根据网上的资料整理出来了，没有官方文档支持，官方文档针对这阶段的描述如下：
```
...
setImmediate()和setTimeout()的调用阶段：
...
V8引擎解析执行JavaScript代码，将异步操作交由event loop处理，然后event loop将任务提交给cpu去执行。由于CPU是为整个操作系统服务器的，所以，CPU同时还可能在运行其他应用，在这样的条件下，就可能有两种情况发生：
...
timeout
```
这段代码和上面唯一不同的地方就是setTimeout和setImmediate都是在一个异步回调中解析执行，执行的过程看下图:
...
- step2：当代码执行到Poll阶段时，如果readFile完成，则会执行readFile的回调函数。
...
// 输出结果： bar 1
```
通过将回调放入到process.nextTick()中，脚本可以全部执行完成，使函数和变量先于回调函数执行。同时还能阻止event loop继续执行。
...
process.nextTick b
```
* Step1：V8引擎解析并执行JavaScript代码，发现有两个setImmediate，放入Check队列中。
...
* 如果在处理nextTickQueue过程中遇到新的nextTick，那么新的nextTick会被添加到当前的nextTickQueue中继续执行。
...
执行顺序：
1. 解析并执行JavaScript代码。
2. JavaScript代码执行到第一行，将promise放入microtask队列。
3. JavaScript代码执行到第五行，将nexttick放入nextTickQueue。
4. JavaScript代码执行到第十五行，将promise放入microtask队列。
5. JavaScript代码执行到第十九行，将nexttick放入nextTickQueue。
6. JavaScript代码执行到第二十四行，将promise放入microtask队列。
7. JavaScript代码执行到第二十八行，将nexttick放入nextTickQueue。
8. 此时microtask队列有三个任务，nextTickQueue有三个任务，JavaScript代码执行完毕，开始event loop阶段。
